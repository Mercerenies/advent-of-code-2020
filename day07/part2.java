...
public class part2 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    DAGNode shinyGold = nodes.get("shiny gold");
    System.out.println(requiredBagsWithin(shinyGold));

  }

  public static int requiredBagsWithin(DAGNode node) {
    int count = 0;
    for (ChildNode c : node.children) {
      // 1 for the child itself, plus all bags within it.
      count += c.amount * (1 + requiredBagsWithin(c.node));
    }
    return count;
  }
